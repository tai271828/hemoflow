...
int main(int argc, char *argv[])
{
    ...
    if(isCheckpointed) {
        ...
        if(saveInitState) {
            ...
            writeHDF5(*lattice, sim, -1, outDir, &openings, porosityField);
        }
        ...
        // writeVTK(*lattice, 0);
        // writeNPZ(*lattice, 0);
        writeHDF5(*lattice, sim, 0, outDir, &openings);
    ...
    while(stat_cycle*sim.C_t <= simLength + sim.C_t)
    {
        ...
        if(stat_cycle % 200 == 0) {
            ...
            if (std::isnan(cE)){
                ...
                writeHDF5(*lattice, sim, stat_cycle, outDir, &openings, porosityField);
                return 0;
            }
            for (auto &o : openings)
            {
