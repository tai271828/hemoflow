...
void writeHDF5(MultiBlockLattice3D<T,DESCRIPTOR>& lattice, const SimPar &sim, plint iter, string outDir, MultiNTensorField3D<T> *field1)
{
    ...
    // Use chunking - adapt chunk size to domain size (must be <= dimension size)
    hsize_t chunk_x = std::min((hsize_t)100, (hsize_t)Nx);
    hsize_t chunk_y = std::min((hsize_t)100, (hsize_t)Ny);
    hsize_t chunk_z = std::min((hsize_t)100, (hsize_t)Nz);
